...
sincronizar_elemento() {
    ...
    local count
    count=$(rsync "${RSYNC_OPTS[@]}" --itemize-changes "$origen" "$destino" 2>&1 \
		| tee -a "$LOG_FILE" \
		| grep -E '^[<>].' \
		| wc -l)
    ...
    if [ $rc -eq 0 ]; then
	    echo "✓ Sincronización completada: $elemento ($count archivos transferidos)"
	    registrar_log "Sincronización completada: $elemento ($count archivos transferidos)"
	    return 0
    else
        if [ $rc -eq 124 ]; then
