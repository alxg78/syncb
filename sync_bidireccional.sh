...
init_log() {
    ...
    echo "Modo: $MODE, Borrado: $DELETE, Simulación: $DRY_RUN, Sobrescribir: $OVERWRITE" >> "$LOG_FILE"
...
generate_symlinks_metafile() {
    ...
    # Busca enlaces simbólicos en directorios clave y los guarda en el archivo temporal.
    # El formato es: ruta_relativa_del_enlace -> destino_absoluto_del_enlace
    find "${LOCAL_DIR}/.local/bin" -type l -print | while read -r symlink; do
        local relative_path=$(realpath --relative-to="${LOCAL_DIR}" "$symlink")
        local target=$(readlink -f "$symlink")
        echo -e "${relative_path}\t${target}" >> "$temp_file"
    done
    ...
    if [ -s "$temp_file" ]; then
        ...
        log_message "No se encontraron enlaces simbólicos para registrar."
    fi
...
recreate_symlinks_from_metafile() {
    ...
    while IFS=$'\t' read -r relative_path target; do
        ...
        if [ -L "$full_path" ] && [ "$(readlink -f "$full_path")" = "$target" ]; then
            continue # Enlace ya existe y es correcto, se omite
        fi
...
run_sync() {
    ...
    # Genera el archivo de metadatos si el modo es "subir"
    if [ "$MODE" = "subir" ]; then
        generate_symlinks_metafile
    fi
    ...
    if [ -n "$ITEM_TO_SYNC" ]; then
        ...
        if [ -z "$SYNC_LIST_FILE" ]; then
            ...
            echo "Asegúrate de tener el archivo ${SYNC_LIST_FILE:-'sync_bidireccional_directorios.ini'} en el directorio del script o en el directorio actual."
            exit 1
        fi
...
if [ -z "$ITEM_TO_SYNC" ]; then
    ...
    if [ -z "$SYNC_LIST_FILE" ]; then
        ...
        echo "Crea un archivo '${HOSTNAME_RTVA}.ini' o 'directorios.ini' con las rutas a sincronizar, o usa la opción --item."
        exit 1
    fi
fi
