...
verificar_pcloud_montado() {
    ...
    if ! mount | grep -q "pCloud\|pcloud"; then
        ...
        echo "Asegúrate de que pCloud Drive esté ejecutándose and montado."
        exit 1
    fi
    ...
    if [ $DRY_RUN -eq 0 ] && [ "$BACKUP_DIR_MODE" = "comun" ]; then
        ...
        if touch "$test_file" 2>/dev/null; then
            ...
            echo "Asegúrate de que pCloud Drive esté funcionando correctamente and tengas permisos de escritura."
            exit 1
        fi
    fi
...
generar_archivo_enlaces() {
    ...
    registrar_enlace() {
        ...
        # Verificar que es un enlace simbólico válido y que existe
        if [ ! -L "$enlace" ] || [ ! -e "$enlace" ]; then
            echo "Advertencia: No es un enlace simbólico válido o está roto - $enlace"
            registrar_log "Advertencia: No es un enlace simbólico válido o está roto - $enlace"
            return
        fi
        
        # Obtener ruta relativa del enlace (respecto a LOCAL_DIR)
        local ruta_relativa=$(realpath --relative-to="${LOCAL_DIR}" "$enlace" 2>/dev/null)
        if [ -z "$ruta_relativa" ]; then
            echo "Advertencia: No se pudo obtener la ruta relativa para - $enlace"
            registrar_log "Advertencia: No se pudo obtener la ruta relativa para - $enlace"
            return
        fi
        
        # Obtener el destino absoluto del enlace
        local destino=$(readlink -f "$enlace" 2>/dev/null)
        if [ -z "$destino" ]; then
            echo "Advertencia: No se pudo resolver el destino del enlace - $enlace"
            registrar_log "Advertencia: No se pudo resolver el destino del enlace - $enlace"
            return
        fi
        
        # Verificar que el destino existe
        if [ ! -e "$destino" ]; then
            echo "Advertencia: El destino del enlace no existe - $enlace -> $destino"
            registrar_log "Advertencia: El destino del enlace no existe - $enlace -> $destino"
            return
        fi
    ...
    if [ -n "$ITEM_ESPECIFICO" ]; then
        ...
        if [ -L "$ruta_completa" ] && [ -e "$ruta_completa" ]; then
